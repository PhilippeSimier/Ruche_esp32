...
char* Weather::getPduAprs() {
    ...
    if (!isnan(speed))    snprintf(sspeed,    sizeof (sspeed),    "%03.0f",  speed);
    if (!isnan(gust))     snprintf(sgust,     sizeof (sgust),     "g%03.0f", gust);
    if (!isnan(temp))     snprintf(stemp,     sizeof (stemp),     "t%03.0f", temp);
    if (!isnan(rain))     snprintf(srain,     sizeof (srain),     "r%03.0f", rain);
    if (!isnan(rain24))   snprintf(srain24,   sizeof (srain24),   "p%03.0f", rain24);
    if (!isnan(rainMid))  snprintf(srainMid,  sizeof (srainMid),  "P%03.0f", rainMid);
